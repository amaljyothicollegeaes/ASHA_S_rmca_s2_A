...
select sum(AMOUNT)from DEPOSIT where CNAME in (select CNAME from CUSTOMER where CITY='NAGPUR') ;

/*6.List maximum deposit of customer living in Bombay*/
select MAX(AMOUNT)from DEPOSIT where CNAME in (select CNAME from CUSTOMER where CITY='BOMBAY') ;
